...
class GridTrader:
    ...
    def start(self) -> bool:
        ...
        try:
            # Get current price
            ticker = self.client._request(f"/v5/market/tickers?category=linear&symbol={self.config.symbol}")
    ...
    def check_and_rebalance(self):
        ...
        try:
            ...
            for order_id, order_info in list(self.active_orders.items()):
                ...
                status = self.client._request(f"/v5/order/realtime?category=linear&orderId={order_id}")
...
def create_auto_grid(client, symbol: str, investment: float = 50.0, num_grids: int = 10) -> Optional[GridTrader]:
    ...
    """
    try:
        ticker = client._request(f"/v5/market/tickers?category=linear&symbol={symbol}")
        if not ticker or 'list' not in ticker:
            return None
