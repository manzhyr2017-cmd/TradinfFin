...
class DepositAccelerationEngine:
    ...
    def _detect_regime(self, ema9, ema21, ema55, adx, close) -> MarketRegime:
        ...
        return MarketRegime.RANGING_NARROW if a < 15 else MarketRegime.RANGING_WIDE
    
    def _interpret_funding(self, fr: float) -> str:
        if not fr:
            return "Neutral"
        if fr > 0.001:
            return "Extremely Bullish (contrarian SHORT)"
        elif fr > 0.0005:
            return "Bullish"
        elif fr < -0.001:
            return "Extremely Bearish (contrarian LONG)"
        elif fr < -0.0005:
            return "Bearish"
        return "Neutral"
