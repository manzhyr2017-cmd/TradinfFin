...
class SelectorService:
    ...
    Output: Primary List (Top 15) & Secondary List (Next 15).
    """
    ...
    async def run_selection_cycle(self):
        ...
        try:
            ...
            logger.info(f"🚀 Top 15 LONGS: {self.primary_list}")
            logger.info(f"📉 Top 15 SHORTS: {self.secondary_list}")
    ...
    def _rank_coins(self, tickers: List[Dict], hot_sectors: List[str]) -> Tuple[List[str], List[str]]:
        ...
        # Take Top 15 of each
        top_longs = [t['symbol'] for t in gainers[:15]]
        top_shorts = [t['symbol'] for t in losers[:15]]
