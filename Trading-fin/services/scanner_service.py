...
class ScannerService:
    ...
    def scan_symbol(self, market_data: MarketData) -> Optional[AdvancedSignal]:
        ...
        elif self.bot.strategy_name == "deposit_accel":
            # Deposit Accel V3 matches its specific analyze() signature
            return self.bot.engine.analyze(
    ...
    async def scan_all(self) -> List[AdvancedSignal]:
        ...
        logger.info(f"🔍 Starting Chunked Scan of {len(symbols)} symbols (3 at a time)...")
        
        # CHUNKED FETCH (3 at a time to avoid rate limits)
        all_results = []
        chunk_size = 3
        ...
        for i in range(0, len(symbols), chunk_size):
            ...
            logger.info(f"⏳ Processing chunk {current_chunk_idx}/{total_chunks}: {', '.join(chunk)}")
            
            tasks = [self.bot.client.get_complete_market_data_async(s) for s in chunk]
            chunk_results = await asyncio.gather(*tasks, return_exceptions=True)
            all_results.extend(chunk_results)
            
            if i + chunk_size < len(symbols):
                logger.info(f"😴 Chunk {current_chunk_idx} done. Sleeping 10s...")
                await asyncio.sleep(10)
        
        fetched = len([r for r in all_results if r and not isinstance(r, Exception)])
        logger.info(f"✅ Chunked Scan Fetched: {fetched}/{len(symbols)} coins")
        ...
        for i, market_data in enumerate(all_results):
            ...
            try:
                ...
                if signal:
                    ...
                    # Use engine's threshold if it's lower (for Turbo modes like Deposit Accel)
                    threshold = self.bot.min_probability
                    # Check for DepositAccel specific phase config threshold
                    if hasattr(self.bot.engine, '_phase_config'):
                        threshold = min(threshold, self.bot.engine._phase_config.min_confluence_pct)
                    elif hasattr(self.bot.engine, 'min_confluence_pct'):
                        threshold = min(threshold, self.bot.engine.min_confluence_pct)
