...
class CompositeScoreEngine:
    ...
    def __init__(self):
        # Веса компонентов — ВСЕ 9 подключены (v5)
        # Перекалиброваны по данным 270 сделок:
        #   MTF  = тренд, главный предиктор (SHORT WR 34% vs LONG 25%)
        #   OI   = кто в рынке, сквизы, ликвидации
        #   SMC  = точка входа
        #   Regime = контекст (тренд vs флэт = разный подход)
        #   OF   = реальное давление (данные показали: слабый предиктор)
        self.weights = {
            'mtf': 0.25,           # 25% — Тренд = главное
            'smc': 0.15,           # 15% — Точка входа
            'orderflow': 0.10,     # 10% — Давление (слабый по данным)
            'volume_profile': 0.10,# 10% — Справедливая цена
            'open_interest': 0.15, # 15% — Кто в позициях
            'regime': 0.10,        # 10% — Контекст рынка
            'whale': 0.05,         # 5%  — Smart Money  
            'fear_greed': 0.05,    # 5%  — Настроение толпы
            'correlation': 0.05    # 5%  — BTC влияние
        }
