...
class CompositeScoreEngine:
    ...
    def __init__(self):
        # Веса компонентов — ТОЛЬКО рабочие (сумма = 1.0)
        # v3: Перераспределено. Было 55% на 3 рабочих + 45% мёртвый груз.
        # Теперь 100% распределено по 3 работающим движкам.
        self.weights = {
            'mtf': 0.40,           # 40% — Тренд = главное (был 20%)
            'smc': 0.35,           # 35% — Точка входа (был 20%)
            'orderflow': 0.25,     # 25% — Реальное давление (был 15%)
            'volume_profile': 0.0, # Не подается — обнулено
            'open_interest': 0.0,  # Не подается — обнулено
            'regime': 0.0,         # Не подается — обнулено
            'whale': 0.0,          # Не подается — обнулено
            'fear_greed': 0.0,     # Не подается — обнулено
            'correlation': 0.0     # Не подается — обнулено
        }
