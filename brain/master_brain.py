...
from core.fastapi_proxy import DashboardAPI
...
class MasterBrain:
    ...
    def _init_core(self):
        ...
        self.reporter = ReportingService(self.db)
    ...
    def _init_state(self):
        ...
        self.protection_lock_until = {} # symbol -> datetime
        self.stats = {
            "decisions_made": 0,
            "trades_executed": 0,
            "blocks_by_module": {},
    ...
    def _level1_survival_check(
        ...
        # ⑥ Freqtrade-style Protections (v1.1)
        is_p, prot_msg = self._check_protections(price)
        if is_p:
            return False, prot_msg

        # ⑦ Периодическая отчетность (Freqtrade-style)
        if time.time() - self._last_report_time > 3600: # Раз в час
            self.reporter.log_full_report()
            self._last_report_time = time.time()

        return True, "✅ Survival OK"

    def _check_protections(self, price: Decimal) -> Tuple[bool, str]:
        """
        Проверка защитных механизмов а-ля Freqtrade.
        Возвращает: (is_protected, reason)
        """
        now = datetime.utcnow()
        symbol = self.current_symbol

        # 1. Проверка активной блокировки в памяти
        until = self.protection_lock_until.get(symbol)
        if until and now < until:
            remaining = (until - now).total_seconds() / 60
            return True, f"🔒 [{symbol}] Cooldown/Lock: {remaining:.1f}m left"

        # 2. Frequent Stoploss Guard (Stage 1)
        lookback = now - timedelta(minutes=config.PROTECTION_STOPLOSS_LOOKBACK_MIN)
        today_trades = self.db.get_trades_today()
        
        # Получаем последние стопы именно для этого символа
        recent_stops = [
            t for t in today_trades 
            if t.get('symbol') == symbol
            and datetime.fromisoformat(t['timestamp']) > lookback 
            and t.get('profit_usdt') is not None 
            and float(t['profit_usdt']) < 0
            and "Stop" in str(t.get('reason', ''))
        ]

        if len(recent_stops) >= config.PROTECTION_STOPLOSS_COUNT:
            lock_time = now + timedelta(minutes=config.PROTECTION_STOPLOSS_LOCK_MIN)
            self.protection_lock_until[symbol] = lock_time
            self.stats["protections_triggered"] += 1
            msg = f"🛡️ StoplossGuard: {len(recent_stops)} stops in {config.PROTECTION_STOPLOSS_LOOKBACK_MIN}m. Locking {symbol} for {config.PROTECTION_STOPLOSS_LOCK_MIN}m."
            self.notifier.send_alert(msg)
            return True, msg

        # 3. Max Daily Loss % Control (Stage 1)
        balance = float(self.client.get_balance())
        daily_pnl = sum(float(t.get('profit_usdt', 0) or 0) for t in today_trades)
        if balance > 0:
            loss_pct = abs(daily_pnl) / balance * 100 if daily_pnl < 0 else 0
            if loss_pct >= config.PROTECTION_MAX_DAILY_LOSS_PCT:
                lock_time = now + timedelta(minutes=1440) # До конца суток условно
                self.protection_lock_until[symbol] = lock_time
                msg = f"🛡️ MaxDrawdownGuard: Daily loss {loss_pct:.2f}% >= {config.PROTECTION_MAX_DAILY_LOSS_PCT}%. Stopping {symbol}."
                self.notifier.send_alert(msg)
                return True, msg

        # 4. Cooldown Period (Stage 1)
        if today_trades and symbol in [t.get('symbol') for t in today_trades]:
            # Берем последний трейд именно этого символа
            symbol_trades = [t for t in today_trades if t.get('symbol') == symbol]
            latest_trade = sorted(symbol_trades, key=lambda t: t['timestamp'])[-1]
            last_close = datetime.fromisoformat(latest_trade['timestamp'])
            
            elapsed_min = (now - last_close).total_seconds() / 60
            if elapsed_min < config.PROTECTION_COOLDOWN_MIN:
                remaining = config.PROTECTION_COOLDOWN_MIN - elapsed_min
                return True, f"🕒 [{symbol}] Cooldown: {remaining:.1f}m left after last trade"
    ...
    def _level8_learn(
        ...
        # Сохраняем причину в БД (Stage 4)
        try:
            self.db.save_trade_reason(order_id, reason)
        except Exception as e:
            log.warning(f"Error saving trade reason: {e}")
    ...
    def _handle_fill(
        ...
        price = Decimal(str(fill_price))
        is_prot, prot_msg = self._check_protections(price)
        if is_prot:
            notes.append(f"🛡️ Protection: {prot_msg}")
        ...
        tech_note = f"RSI:{snap.rsi:.1f}, Vol:{snap.volatility_pct:.2f}%, Trend:{snap.ml_regime}"
        notes.append(f"L2: {tech_note}")
        
        # Собираем финальную причину из всех уровней (Stage 4)
        full_reason = " | ".join(notes)
        self._level8_learn(order_id, side, fill_price, qty, profit, level_index, reason=full_reason)
