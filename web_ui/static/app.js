...
async function fetchHistory() {
    try {
        ...
        if (!data.trades || data.trades.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding: 2rem; color: #64748b;">Истории пока нет</td></tr>';
            return;
        }
...
async function sendChatMessage() {
    ...
    try {
        ...
        if (data.response) {
            addChatMessage('Neuro-Bot', data.response, 'bot');
        } else {
            addChatMessage('System', 'Error receiving response', 'bot');
        }
    } catch (e) {
        addChatMessage('System', 'Connection error', 'bot');
    }
}
...
function addChatMessage(sender, text, type) {
    ...
    // Markdown-like bolding for **text** if we want simple parsing
    const formattedText = text.replace(/\n/g, '<br>').replace(/\*\*(.*?)\*\*/g, '<b>$1</b>');

    div.innerHTML = `
        <span class="prefix">${sender}:</span>
